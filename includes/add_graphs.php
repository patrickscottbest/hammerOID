...
if (sizeof($parms)) {
	...
	foreach($parms as $parameter) {
		...
		case '--input-fields':
			...
			break;
		case '--snmp-query-id':
			$dsGraph['snmpQueryId'] = $value; // sent 7

			break;
		case '--snmp-query-type-id':
			$dsGraph['snmpQueryType'] = $value; //sent 23

			break;
		case '--snmp-field':
			$dsGraph['snmpField'][] = $value; // sent "hostid" 
		...
		case '--snmp-value-regex':
			...
			break;
		case '--snmp-value':
			$dsGraph['snmpValue'][] = $value; // sent "5"
		...
		case '--reindex-method':
			if (is_numeric($value) &&
				...
				$dsGraph['reindex_method'] = $value; 			// sent "0"
			} else {
